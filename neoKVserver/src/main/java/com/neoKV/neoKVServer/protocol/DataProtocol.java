...
public class DataProtocol implements Protocol {
    ...
    // format : [TOTAL_SIZE(4 byte)][TIMESTAMP(8 byte)][TOMBSTONE(1 byte)][KEY_SIZE(4 byte)][KEY_BYTES_LENGTH(key bytes length)][VALUE_WITH_DATA_TYPE()]
    public ByteBuffer allocateDirect(Set<Map.Entry<String, byte[]>> data) {
        ...
        for (Map.Entry<String, byte[]> entry : data) {
            ...
            totalLength += Constants.TIMESTAMP_BYTE_LENGTH;
     ...
     * @param byteBuffer
     * @param keyBytes
     * @param values : tombstone + timestamp + data type + data bytes
     */
